...
namespace PetsonalAI {
	public class ContactModule : Module {
		...
		public IEnumerator CaptureContactsAsync(MotionEditor editor, UnityAction<float> updateProgress) {
			...
			for (var i = 0; i < Asset.Frames.Length; i++)
			{
				...
				updateProgress?.Invoke(Asset.Frames.Length > 1 ? (i/(float)(Asset.Frames.Length - 1)) : 1);
				
				if (Utility.GetElapsedTime(timestamp) >= 0.2f)
				{
