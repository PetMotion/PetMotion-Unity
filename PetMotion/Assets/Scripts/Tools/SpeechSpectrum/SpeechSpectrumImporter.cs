...
namespace PetsonalAI {
	public class SpeechSpectrumImporter : BatchProcessor {

		public string Source = string.Empty;
		public string Destination = string.Empty;
		public int Framerate = 60;

		[MenuItem("PetsonalAI/Importer/Speech Spectrum Importer")]
		static void Init() {
			Window = EditorWindow.GetWindow(typeof(SpeechSpectrumImporter));
			Scroll = Vector3.zero;
		}
		...
		public override IEnumerator DerivedProcess(Item item) {
			...
			SpeechSpectrum asset = AssetDatabase.LoadAssetAtPath<SpeechSpectrum>(featuresDestination + "/" + rawFileName + ".asset");
			if (asset == null) {
				asset = ScriptableObjectExtensions.Create<SpeechSpectrum>(featuresDestination, rawFileName, true);
			}
			...
			if (!ArrayExtensions.Same(spectogram.Length, beats.Length, flux.Length)) {
				...
				asset.Samples = new SpeechSpectrum.Sample[spectogram.Length];
				for (int i = 0; i < asset.Samples.Length; i++) {
					asset.Samples[i] = new SpeechSpectrum.Sample(spectogram[i], beats[i], flux[i], mfcc[i], chroma[i], zeroCrossing[i]);
				}
			}
